  ...
  line.forEach(({ direction, amount }, commandIdx) => {
    ...
    for (let i = 1; i <= amount; ++i) {
      ...
      if (Object.keys(value).length === lines.length) {
        crossingPoints.push({ x: coords[X], y: coords[Y], value });
      }
    }
  });
});
...
const smallestDistance = crossingPoints.reduce((smallest, { value }) => {
  const distance = Object.values(value).reduce((sum, v) => sum + v, 0);
